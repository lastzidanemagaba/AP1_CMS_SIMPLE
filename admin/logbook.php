        ...
        <div class="row mb-2">
          ...
          <!--div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Logbook <?php //echo get_project();?></li>
            </ol>
          </div-->
        </div>
      </div><!-- /.container-fluid -->
            ...
            <div class="card">
              ...
              <div class="card-body">
                ...
                <table id="example1" class="table table-bordered table-hover">
				  ...
				  <?php
					if($awal == ""){
						$result = mysqli_query($con,"SELECT logbook_report.id ID, logbook_report.uraian_kegiatan KEGIATAN, 
						logbook_report.hasil_pengawasan PENGAWASAN,logbook_report.create_date WAKTU, level_shift.shift SHIFT,
						user_list.nama_lengkap NAMA
						FROM logbook_report
						LEFT JOIN user_list on user_list.id = logbook_report.create_by
						LEFT JOIN level_shift on level_shift.id = logbook_report.id_shift
						ORDER BY logbook_report.id DESC LIMIT 100");
					}else if($awal != ""){
						$result = mysqli_query($con,"SELECT logbook_report.id ID, logbook_report.uraian_kegiatan KEGIATAN, 
						logbook_report.hasil_pengawasan PENGAWASAN,logbook_report.create_date WAKTU, level_shift.shift SHIFT,
						user_list.nama_lengkap NAMA
						...
						LEFT JOIN user_list on user_list.id = logbook_report.create_by
						LEFT JOIN level_shift on level_shift.id = logbook_report.id_shift
						WHERE (DATE(logbook_report.create_date) BETWEEN '$awal' AND '$akhir')
						ORDER BY logbook_report.id DESC LIMIT 100");
					}
					...
					while($row = mysqli_fetch_array($result)){
						...
						if(session_level()!="0"){
							...
							<td align='center'>
								<b>
								<a href='logbook_act?logbook=".md5($row['ID'])."&status=".md5("hapus")."' class='btn btn-danger btn-small' onClick=\"return confirm('Do you want delete this data ?')\" >Delete</a>
								</b>
							</td>
						</tr>";
						}
